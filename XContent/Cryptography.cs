...
{
    public class HorizonCrypt
    {
        public static void memset(byte[] Input, int index, int value, int num)
        {
            if (Input != null && (index + num) <= Input.Length)
            {
                for (var x = index; x < (index + num); x++)
                {
                    Input[x] = Convert.ToByte(value);
                }
            }
        }
        ...
        public static byte[] XeCryptSha(byte[] pbInp1, byte[] pbInp2, byte[] pbInp3)
        {
            return XeCryptSha(pbInp1, pbInp1.Length, pbInp2, pbInp2 == null ? 0 : pbInp2.Length, pbInp3, pbInp3 == null ? 0 : pbInp3.Length);
        }
        public static byte[] XeCryptSha(byte[] pbInp1, int cbInp1, byte[] pbInp2, int cbInp2, byte[] pbInp3, int cbInp3)
        {
            ...
            sha.TransformBlock(pbInp1, 0, cbInp1, null, 0);

            if (pbInp2 != null)
                sha.TransformBlock(pbInp2, 0, cbInp2, null, 0);
            if (pbInp3 != null)
                sha.TransformBlock(pbInp3, 0, cbInp3, null, 0);

            sha.TransformFinalBlock(pbInp1, 0, 0);
            return sha.Hash;
        }
        public static byte[] XeCryptHmacSha(byte[] pbKey, int cbkeylen, byte[] pbInp1, int cbInp1, byte[] pbInp2, int cbInp2, byte[] pbInp3, int cbInp3)
        {
            if (pbInp1 == null || pbInp1.Length == 0)
            {
                throw new Exception("Attempted to hash an invalid buffer.");
        ...
        {
            RSACryptoServiceProvider RsaCryptProvider = new RSACryptoServiceProvider();

            RsaCryptProvider.ImportParameters(RSAParameters);

            RSAPKCS1SignatureFormatter SigFormatter = new RSAPKCS1SignatureFormatter(RsaCryptProvider);

            SigFormatter.SetHashAlgorithm("SHA1");

            byte[] Signature = SigFormatter.CreateSignature(SHA1(Input));

            Array.Reverse(Signature);

            return Signature;
        }
        public static bool IsBufferNull(byte[] Buffer)
        {
        ...
        public static byte[] XeKeysUnObfuscate(int Index, byte[] EncryptedBuffer, bool Dev)
        {
            ...
            int tempIndex = (0 - Index);
            int KeyIndex = ((tempIndex + tempIndex) & 2) + 0x17;

            byte[] BaseKey = new byte[0x10];
            byte[] Body = new byte[BufferSize - 0x10];
            ...
            if (!ArrayEquals(Hmac, BaseKey))
                return null;

            byte[] RealData = new byte[BufferSize - 0x18];
        ...
        public static byte[] XeKeysObfuscate(int Index, byte[] Buffer, bool Dev)
        {
            ...
            byte[] Confounder = new byte[0x8];
            new Random().NextBytes(Confounder);
            byte[] Body = new byte[BufferSize + 8];
        ...
        public static byte[] XeKeysHmacSha(int KeyIndex, byte[] Buffer, int BufferSize, int DigestSize, bool Dev)
        {
            HMACSHA1 hmac = new HMACSHA1(HvpGetKey(KeyIndex, Dev ));
        ...
        public static byte[] HvpGetKey(int KeyIndex, bool Dev)
        {
            if (!Dev)
            {
                switch (KeyIndex)
                {
                    case 0x19:
                        return new byte[0x10] { 0xE1, 0xBC, 0x15, 0x9C, 0x73, 0xB1, 0xEA, 0xE9, 0xAB, 0x31, 0x70, 0xF3, 0xAD, 0x47, 0xEB, 0xF3 };
                }
            }
            else
            {
                ...
                {
                    case 0x19:
                        return new byte[0x10] { 0xDA, 0xB6, 0x9A, 0xD9, 0x8E, 0x28, 0x76, 0x4F, 0x97, 0x7E, 0xE2, 0x48, 0x7E, 0x4F, 0x3F, 0x68 };
                }
            }
            return null;
        }
        ...
        public static bool VerifyRSASignature(RSACryptoServiceProvider RSAProvider, byte[] Signature, byte[] Data)
        {
            RSAPKCS1SignatureDeformatter RSA = new RSAPKCS1SignatureDeformatter(RSAProvider);
            byte[] SHA1 = SHA1CryptoServiceProvider.Create().ComputeHash(Data);
            RSA.SetHashAlgorithm("SHA1");
            return RSA.VerifySignature(SHA1, Signature);
        }
        public static byte[] ReverseQword(byte[] Input)
        {
